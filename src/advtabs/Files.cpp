...
FileListView::FileListView(do_ui_update_fn uiu, HWND hwnd) :
	ini_class_t(L"listviews/adv_files", L"file_listview"),
	do_ui_update_(uiu),
	treeview_(hwnd)
{}
...
{	
	...
	for (auto i = begin_selected(), e = end_selected(); i != e; ++i)
	{
		HAL_DEV_MSG(hal::wform(L"OnMenuPriority() = %1%, %2%, %3%") 
				% std::wstring(i->text()) % i->index() % files_[i->index()].order());
		indices.push_back(numeric_cast<int>(files_[i->index()].order()));
	}
...
AdvFilesDialog::AdvFilesDialog(HaliteWindow& halWindow) :
	...
	treeManager_(tree_),
	ini_class_t(L"adv_files_dialog", L"settings"),
	splitterPos(150),
...
void AdvFilesDialog::uiUpdate(const hal::torrent_details_manager& tD)
{
	...
	if (hal::try_update_lock<FileListView::list_class_t> lock{ &list_ })
	{
		hal::file_details_vec all_files = focused_torrent()->get_file_details();	
		FileListView::scoped_files list_files = list_.files();
