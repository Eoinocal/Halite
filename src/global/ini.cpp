...
public:
	...
	{
		wpath backup = app().working_directory()/(filename + L".pre")
		
		if (boost::filesystem::exists(backup))
			boost::filesystem::boost::filesystem(backup);
		
		boost::filesystem::copy_file(app().working_directory()/filename, backup);
	}
